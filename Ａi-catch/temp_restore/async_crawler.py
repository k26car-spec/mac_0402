...
class AsyncStockCrawler:
    ...
    async def fetch_yahoo_quote(self, stock_code: str) -> Dict:
        ...
        try:
            async with self.session.get(url, params=params, headers=self.headers, ssl=self.ssl_context, timeout=10) as response:
                if response.status != 200:
                    return {}
            ...
            logger.error(f"Yahoo Quote 獲取錯誤 {stock_code}: {e}")
            return {}
