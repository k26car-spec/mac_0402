...
cleanup() {
    ...
    lsof -ti :8000,3000,5173,8888,8501 | xargs kill -9 2>/dev/null
...
if [ -d "$ROOT_DIR/../day-trading-orb" ]; then
    ...
    echo "   ⚠️ 找不到 day-trading-orb 目錄 ($ROOT_DIR/../day-trading-orb)，跳過"
fi

# 6. 啟動管理頁面 (Port 8888)
echo -e "${CYAN}⚙️  [5/6] 啟動清單管理工具 (Port 8888)...${NC}"
kill -9 $(lsof -ti:8888) 2>/dev/null
cd "$ROOT_DIR/static"
nohup python3 -m http.server 8888 > "$LOG_DIR/static.log" 2>&1 &
echo -e "   ${GREEN}✅ 管理頁面已啟動${NC}"

# 7. 啟動 Streamlit 潛力股監控儀表板 (Port 8501)
echo -e "${CYAN}📈 [6/6] 啟動 Streamlit 潛力股監控 (Port 8501)...${NC}"
...
nohup streamlit run streamlit_stock_monitor.py --server.port 8501 --server.headless true > "$LOG_DIR/streamlit.log" 2>&1 &
echo -e "   ${GREEN}✅ Streamlit 監控儀表板已啟動${NC}"
...
echo -e "   📈 ${CYAN}潛力股監控儀表:${NC}  http://localhost:8501"
